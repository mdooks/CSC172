...
public String[] tokenize(String t){ //split the string up into its tokens
  String[] res = new String[t.length()]; //set up an array to store them, there cannot be more tokens than there are chars in the string so this works.
  ...
  for (int i = 0; i<t.length(); i++){ //for each char
    ...
    if(String.valueOf(t.charAt(i)).equals("-") && (!(Calc.operator(String.valueOf(t.charAt(i+1)))) && !(String.valueOf(t.charAt(i+1)).equals(" ")))){
      //System.out.println((String.valueOf(t.charAt(i+1))) + "i+1");
      //System.out.println(!(String.valueOf(i+1).equals(" ")));
      //System.out.println("negative");
      temp = (String.valueOf(t.charAt(i)) + String.valueOf(t.charAt(i+1)));
